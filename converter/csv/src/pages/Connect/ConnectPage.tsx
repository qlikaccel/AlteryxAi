...
export default function ConnectPage() {
  ...
  const selectedFileArray = selectedFiles ? Array.from(selectedFiles) : [];
  const selectedWorkflowCount = selectedFileArray.filter((file) => /\.(yxmd|yxzp|zip)$/i.test(file.name)).length;
  const selectedMacroCount = selectedFileArray.filter((file) => /\.yxmc$/i.test(file.name)).length;
...
<div className="connection-methods">
  ...
  >
    ...
    <span className="method-subtitle">.yxmd, .yxmc, .yxzp, .zip</span>
  </button>
</div>
          ...
          <>
            <div className="field-group">
              ...
              <p className="field-hint">
                For macro workflows, upload the parent .yxmd together with all referenced .yxmc files, preferably as a single .yxzp package.
              </p>
            </div>

            {selectedFiles?.length ? (
              <div className="upload-file-summary">
                <strong>{selectedFiles.length} file(s) selected for assessment.</strong>
                <span>{selectedWorkflowCount} workflow/package file(s)</span>
                <span>{selectedMacroCount} macro file(s)</span>
              </div>
            ) : null}
          </>
        )}
