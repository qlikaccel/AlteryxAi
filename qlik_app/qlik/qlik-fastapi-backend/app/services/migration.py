...
#     # ✅ FIX: Alteryx source_fields_map — maps _raw table name → list of field dicts
#     # {"name": "col", "type": "string"}.  Populated from convert_workflow_to_m's
#     # source_fields_map and forwarded by the frontend on every publish call.
#     # Allows BIM builder to emit real column definitions for _raw CSV tables
#     # whose Alteryx workflow JSON carries no field-level schema.
...
#     # When the Alteryx workflow JSON has no field-level schema for source CSV
#     # files, the generated M query uses a dynamic List.Transform pattern that
#     # contains no static column names.  The BIM builder cannot extract column
#     # definitions from it, so those _raw tables appear empty in Power BI.
#     #
#     # Fix: the frontend forwards mquery.source_fields_map (built by
#     # convert_workflow_to_m) in request.alteryx_source_fields.  We apply it
...
#             tname = t.get("name", "")
#             if t.get("fields"):
#                 continue  # already has columns — don't overwrite
#             map_entry = alteryx_source_fields.get(tname, [])
#             if not map_entry:
#                 continue
...
#                     continue
#                 if col_name:
#                     normalised.append({"name": col_name, "alias": col_name,
#                                        "expression": col_name, "type": col_type,
