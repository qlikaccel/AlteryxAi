...
def convert_workflow_to_m(
    ...
    # files carry no schema in the Alteryx workflow JSON (fields_count == 0).
    #
    # FIX: When source.fields is empty, mine the workflow's Select/Summarize
    # nodes to infer columns that flow from each source table. This ensures
    # Power BI always receives static column definitions for _raw tables.
    source_fields_map: dict[str, list[dict[str, Any]]] = {}

    def _infer_source_columns_from_workflow(wf: dict[str, Any]) -> list[dict[str, str]]:
        """
        Walk Select, Summarize, Formula, Join nodes and collect every named
        column/field definition visible in the workflow graph. Used as a
        fallback when the source dataSources entry has no fields list.
        """
        inferred: dict[str, str] = {}  # name -> type, preserving first-seen order
        nodes = wf.get("workflowNodes") or []
        for node in nodes:
            tool = str(node.get("plugin") or node.get("tool") or "").lower()
            config = node.get("config") if isinstance(node.get("config"), dict) else {}

            # Select tool: has selectedFields with name+type
            selected = config.get("selectedFields") if config else None
            ...
            # Summarize/Aggregate: groupBy fields + aggregations
            group_by = config.get("groupBy") if config else None
            if isinstance(group_by, list):
                for col in group_by:
                    col = str(col).strip()
                    if col and col not in inferred:
                        inferred[col] = "string"
            aggregations = config.get("aggregations") if config else None
            if isinstance(aggregations, list):
                for agg in aggregations:
                    if not isinstance(agg, dict):
                        continue
                    col = str(agg.get("field") or agg.get("rename") or "").strip()
                    typ = "number" if str(agg.get("action") or "").lower() in {
                        "sum", "avg", "min", "max", "count"
                    } else "string"
                    if col and col not in inferred:
                        inferred[col] = typ

            # Formula tool: formula output fields
            formulas = config.get("formulas") if config else None
            if isinstance(formulas, list):
                for formula in formulas:
                    if not isinstance(formula, dict):
                        continue
                    col = str(formula.get("field") or "").strip()
                    typ = str(formula.get("type") or "string").strip()
                    if col and col not in inferred:
                        inferred[col] = typ

            # Fallback: parse configurationText for field names (name	type patterns)
            cfg_text = str(node.get("configurationText") or "")
            if cfg_text and "select" in tool:
                for line in cfg_text.splitlines():
                    parts = [p.strip() for p in line.split("\t")]
                    if len(parts) >= 2 and parts[0] and parts[1].lower() in {
                        "string", "int", "int64", "float", "double", "bool",
                        "date", "datetime", "v_string", "v_wstring", "wstring"
                    }:
                        col, typ = parts[0], parts[1]
                        if col not in inferred:
                            inferred[col] = typ

        return [{"name": k, "type": v} for k, v in inferred.items()] if inferred else []
    ...
    for rq in raw_source_queries:
        ...
        # FIX: If the source has no field schema, infer columns from workflow nodes
        if not rq_fields:
            rq_fields = _infer_source_columns_from_workflow(workflow)
        if rq_name:
            source_fields_map[rq_name] = rq_fields
