...
from routers.alteryx_router import router as alteryx_router
from app.routers.context_engineering_router import router as context_engineering_router
...
async def download_pdf(payload: dict = Body(...)):
    ...
    try:
        ...
        # Difference Details section (shown only if there are discrepancies)
        ...
        # Footer
        footer_text = f"""
        <font size=7><i>Report generated by Alteryx to Power BI Migration Tool | {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}</i></font><br/>
