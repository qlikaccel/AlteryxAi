...
export default function PublishPage() {
  ...
  const publishMode =
    ...
    "M_QUERY";
  const isBigQueryPublish = publishMode === "DBT_BIGQUERY" || publishMode === "DATAFORM_BIGQUERY";
  const isDataformPublish = publishMode === "DATAFORM_BIGQUERY";
  ...
  const gcpUrl = isDataformPublish
    ? (publishResult?.dataform_console_url || bigQueryTableUrl(bigQueryFinalModel))
    : bigQueryTableUrl(bigQueryFinalModel);
  ...
  const steps = [
    ...
    { label: isDataformPublish ? "Dataform gen" : isBigQueryPublish ? "dbt gen" : "M Query gen", complete: true },
    { label: isDataformPublish ? "BQ publish" : isBigQueryPublish ? "BQ publish" : "Publish", complete: true },
  ];
    ...
    <div className="publish-shell">
      <header className="publish-topbar">
        <div>
          ...
          <p style={{ margin: 0, fontSize: "1.22rem", fontWeight: 700, color: "#080e17" }}>
            {workflowName} - {isDataformPublish ? "Dataform Published" : "Published"}
          </p>
        </div>
        <div className="publish-top-actions">
          <button className="dark-btn" onClick={openPublishTarget}>
            {isDataformPublish ? "Open In GCP (Dataform)" : isBigQueryPublish ? "Open In GCP" : "Open In Power BI"}
          </button>
        </div>
      </header>
            ...
            {isBigQueryPublish ? (
              <table className="publish-validation-table">
                <tbody>
                  ...
                  <tr>
                    <td>{isDataformPublish ? "Dataform actions" : "dbt commands"}</td>
                    <td>
                      {isDataformPublish
                        ? `${publishResult?.succeeded_actions ?? 0}/${(publishResult?.succeeded_actions ?? 0) + (publishResult?.failed_actions ?? 0)} succeeded`
                        : `${publishResult?.commands?.filter((command: any) => command.success).length || 0}/${publishResult?.commands?.length || 0} succeeded`}
                    </td>
                  </tr>
                  {isDataformPublish && publishResult?.repository_id && (
                    <tr>
                      <td>Dataform repository</td>
                      <td>{publishResult.repository_id}</td>
                    </tr>
                  )}
                  <tr>
      ...
      {conversionSteps.length > 0 && (
        <section className="wire-card tool-mapping-card">
          ...
          <p>
            {isDataformPublish
              ? "Tool conversion mapping from Alteryx workflow to Dataform SQLX"
              : "Tool conversion mapping from Alteryx workflow to Power Query"}
          </p>
          <div className="mapping-table-wrap">
            <table className="tool-mapping-table">
              <thead>
                <tr>
                  <th>Alteryx Tool</th>
                  <th>{isDataformPublish ? "Dataform / BigQuery Mapping" : "Power Query Mapping"}</th>
                </tr>
              </thead>
              <tbody>
