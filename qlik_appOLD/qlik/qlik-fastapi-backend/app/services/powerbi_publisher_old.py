...
def _resolve_table_source_path(base_path: str, source_path: str, source_type: str) -> str:
    """
    🔥 CRITICAL FIX for CSV intermediate tables appearing empty in Power BI
    
    Resolves the full file path for a CSV table by combining base_path with source_path.
    This ensures intermediate CSV tables can properly load their data.
    
    Handles:
    - SharePoint URLs: https://company.sharepoint.com/...
    - S3 URLs: https://bucket.s3.amazonaws.com/...
    - Azure Blob: https://account.blob.core.windows.net/...
    - Web URLs: https://example.com/...
    - Local paths: C:/data/... or /data/...
    
    Returns the properly formatted path for use in the M expression.
    """
    ...
    # If source_path is already a full URL or absolute path, return it as-is
    if source_path.startswith("http://") or source_path.startswith("https://") or \
       source_path.startswith("/") or (len(source_path) > 2 and source_path[1] == ":"):
        return f'"{source_path}"'
    
    # SharePoint URLs
    if "sharepoint.com" in base_path.lower() or "-my.sharepoint.com" in base_path.lower():
        # For SharePoint, source_path is typically the filename
        # The M expression should already handle the file lookup via SharePoint.Files()
        # Return just the filename in quotes for the M expression
        return f'"{source_path}"'
    
    # S3 URLs
    elif "s3.amazonaws.com" in base_path.lower() or ".s3." in base_path.lower():
        base_clean = base_path.rstrip("/")
        combined = f"{base_clean}/{source_path}"
        return f'"{combined}"'
    
    # Azure Blob URLs
    elif "blob.core.windows.net" in base_path.lower() or "dfs.core.windows.net" in base_path.lower():
        base_clean = base_path.rstrip("/")
        combined = f"{base_clean}/{source_path}"
        return f'"{combined}"'
    
    # Web URLs (generic HTTPS)
    elif base_path.lower().startswith("https://") or base_path.lower().startswith("http://"):
        base_clean = base_path.rstrip("/")
        combined = f"{base_clean}/{source_path}"
        return f'"{combined}"'
    
    # Local file paths - use absolute path construction
    else:
        base_clean = base_path.rstrip("/").rstrip("\\")
        source_clean = source_path.lstrip("/").lstrip("\\")
        # Construct path with proper separator
        if "\\" in base_clean:
            combined = f"{base_clean}\\{source_clean}"
        else:
            combined = f"{base_clean}/{source_clean}"
        return f'"{combined}"'
...
class _Publisher:
    ...
    def _build_bim(
        ...
        for t in tables_m:
            ...
            # 🔥 CRITICAL FIX: Resolve CSV intermediate table file paths
            # For CSV tables with source_path but no hardcoded file path in M expression,
            # ensure the M expression properly references the data source.
            # This fixes the issue where intermediate tables load empty in Power BI.
            source_path = t.get("source_path", "").strip()
            if source_path and source_type == "csv" and data_source_path:
                # Check if the M expression uses [DataSourcePath] parameter
                if "[DataSourcePath]" in fixed_expr or "[DataSourcePath]" in expr_str:
                    # Replace parameter with actual resolved path for this specific table
                    resolved_path = _resolve_table_source_path(
                        data_source_path, source_path, source_type
                    )
                    fixed_expr = fixed_expr.replace("[DataSourcePath]", resolved_path)
                    logger.info(
                        "[BIM] '%s': Resolved CSV file path from [DataSourcePath] to %s",
                        table_name, resolved_path[:100]
                    )
                # Also check for cases where File.Contents() or similar functions might need path injection
                elif "File.Contents(" in fixed_expr and source_path:
                    logger.info(
                        "[BIM] '%s': CSV table detected with File.Contents() - verifying path resolution",
                        table_name
                    )
            elif source_type == "csv" and not source_path:
                logger.warning(
                    "[BIM] '%s': CSV table with no source_path - M expression may not load data properly",
                    table_name
                )
