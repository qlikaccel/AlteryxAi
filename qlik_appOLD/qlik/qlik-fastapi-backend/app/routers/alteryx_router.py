...
from app.services.alteryx_dataform_publisher import publish_dataform_to_bigquery
...
def publish_alteryx_workflow_dataform_to_bigquery(
    ...
    """Convert the Alteryx workflow to Dataform SQLX, publish to GCP Dataform,
    execute the invocation, and write the final model into BigQuery."""
    workflow = _find_batch_workflow(batch_id, workflow_id)
    if workflow.get("isMacroDefinition"):
        raise HTTPException(
            status_code=400,
            detail=(
                "Select the parent .yxmd workflow for Publish Dataform to BigQuery. "
                "A .yxmc macro definition is an internal reusable component and does not contain "
                "the external source table mapping needed for Dataform publish."
            ),
        )
    project = generate_dbt_project(workflow, sharepoint_url=sharepoint_url, file_name=file_name)
    try:
        result = publish_dataform_to_bigquery(project)
    except ValueError as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from exc
    except RuntimeError as exc:
        raise HTTPException(status_code=500, detail=str(exc)) from exc
    except Exception as exc:
        logger.exception("Failed to publish Dataform project to BigQuery")
