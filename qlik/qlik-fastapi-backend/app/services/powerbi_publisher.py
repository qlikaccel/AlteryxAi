...
def _acquire_sharepoint_token(sharepoint_url: str = "") -> str:
    """
    Acquire an access token specifically for SharePoint API access.
    
    Uses the correct scope format:
    - For tenant.sharepoint.com: https://tenant.sharepoint.com/.default
    - Falls back to generic SharePoint scope if tenant not provided
    
    Returns: Access token or empty string if acquisition fails
    """
    try:
        import msal
        
        # Extract tenant from SharePoint URL if provided
        # e.g., https://sorimtechnologies.sharepoint.com/sites/Data -> sorimtechnologies.sharepoint.com
        if sharepoint_url:
            url_match = re.search(r'https://([^/]+\.sharepoint\.com)', sharepoint_url)
            if url_match:
                sharepoint_host = url_match.group(1)
                scope = f"https://{sharepoint_host}/.default"
            else:
                # Fallback to generic SharePoint scope
                scope = "https://graph.microsoft.com/.default"
        else:
            # Generic SharePoint scope
            scope = "https://graph.microsoft.com/.default"
        ...
        if not all([tenant_id, client_id, client_secret]):
            logger.debug("[SharePoint Auth] Credentials missing, cannot acquire token")
            return ""
        ...
        if token:
            logger.info("[SharePoint Auth] Token acquired for scope: %s", scope)
        else:
            error_msg = result.get("error_description", "unknown error")
            logger.warning("[SharePoint Auth] Token acquisition failed: %s", error_msg)
        
        return token
    except Exception as exc:
        logger.error("[SharePoint Auth] Exception during token acquisition: %s", exc, exc_info=True)
        return ""
...
def _extract_sharepoint_csv_path(expr_str: str) -> tuple[Optional[str], Optional[str]]:
    """
    Extract SharePoint URL and CSV file path from M expression.
    
    Looks for patterns like:
    - SharePoint.Files("https://...", [ApiVersion=15])
    - Csv.Document(..., [Headers=1])
    - Parameters!DataSourcePath value
    
    Returns: (sharepoint_url, csv_file_path) or (None, None)
    """
    sharepoint_url = None
    csv_path = None
    
    # Pattern 1: Extract SharePoint.Files URL
    sp_match = re.search(
        r'SharePoint\.Files\s*\(\s*"([^"]*sharepoint[^"]*)"',
        expr_str,
        re.IGNORECASE
    )
    if sp_match:
        sharepoint_url = sp_match.group(1).strip()

    
    # ✅ NEW: Pattern for Table.SelectRows [Name] = "filename.csv"
    name_match = re.search(
        r'\[Name\]\s*=\s*"([^"]+\.csv)"',
        expr_str, re.IGNORECASE
    )
    if name_match:
        csv_path = name_match.group(1).strip()
        return (sharepoint_url, csv_path)
    
    # ✅ NEW: Pattern for Text.EndsWith([Name], "filename.csv")
    ends_match = re.search(
        r'Text\.EndsWith\s*\([^,]+,\s*"([^"]+\.csv)"',
        expr_str, re.IGNORECASE
    )
    if ends_match:
        csv_path = ends_match.group(1).strip()
        return (sharepoint_url, csv_path)
    
    # Pattern 2: Extract Csv.Document source (may be file path or variable)
    csv_match = re.search(
        r'Csv\.Document\s*\(\s*([^,]+)\s*,',
        expr_str
    )
    if csv_match:
        csv_source = csv_match.group(1).strip()
        # If it's a quoted string, extract the path
        if csv_source.startswith('"') and csv_source.endswith('"'):
            csv_path = csv_source[1:-1]
        # If it's a variable like #"Headers" or Source, we'll need to trace it
        elif csv_source.startswith('#"'):
            csv_path = csv_source
        elif csv_source.startswith('Source'):
            csv_path = 'Source'
    
    # Pattern 3: Look for file path hints in File.Contents or Web.Contents
    content_match = re.search(
        r'(?:File\.Contents|Web\.Contents)\s*\(\s*"([^"]+\.csv)"',
        expr_str,
        re.IGNORECASE
    )
    if content_match and not csv_path:
        csv_path = content_match.group(1)
    
    return (sharepoint_url, csv_path)


def _fetch_csv_headers_from_sharepoint(
    sharepoint_url: str,
    csv_file_path: str,
    access_token: str = "",
) -> List[str]:
    """
    Fetch CSV headers from SharePoint using direct HTTP GET.
    
    Attempts to:
    1. Download CSV from SharePoint using provided or acquired token
    2. Parse CSV headers with csv.reader (handles quoted columns)
    3. Fallback to simple comma-split parser if csv.reader fails
    4. Try multiple token scopes if SharePoint token not provided
    
    Returns: List of column names from CSV headers
    """
    try:
        if not sharepoint_url:
            logger.debug("[CSV Headers] No SharePoint URL provided")
            return []
        
        # Normalize SharePoint URL
        sp_base = sharepoint_url.rstrip('/')
        if '/sites/' not in sp_base and '/teams/' not in sp_base:
            logger.debug("[CSV Headers] Invalid SharePoint URL format: %s", sp_base)
            return []
        
        # Build the CSV download URL
        # CSV path might be like "data.csv", "/data.csv", or "Shared Documents/data.csv"
        csv_file_clean = str(csv_file_path).lstrip('/').strip()
        
        # Handle common SharePoint paths
        if not csv_file_clean.lower().startswith('shared'):
            # If path doesn't start with "Shared", assume it's in Shared Documents
            download_url = f"{sp_base}/Shared%20Documents/{csv_file_clean}"
        else:
            download_url = f"{sp_base}/{csv_file_clean.replace(' ', '%20')}"
        
        logger.info(
            "[CSV Headers] Attempting to fetch from SharePoint: %s (file: %s)",
            download_url[:120], csv_file_clean
        )
        ...
        # Try direct download with timeout
        resp = requests.get(
            download_url,
        ...
        logger.info("[CSV Headers] Response status: %d", resp.status_code)
        
        if resp.status_code == 200:
            # Parse CSV content and extract headers
            if not resp.text or resp.text.strip() == "":
                logger.warning("[CSV Headers] CSV file is empty")
                return []
            
            try:
                # Try RFC 4180 CSV parsing first (handles quoted columns, escapes, etc.)
                csv_file_obj = io.StringIO(resp.text)
                reader = csv.reader(csv_file_obj)
                headers = next(reader)
                logger.info(
                    "[CSV Headers] ✅ Successfully fetched %d headers via RFC 4180 CSV parser",
                    len(headers)
                )
                return headers
            except StopIteration:
                logger.warning("[CSV Headers] CSV file appears empty in parser")
                return []
            except Exception as parse_exc:
                # Fallback to simple comma split
                logger.debug("[CSV Headers] RFC 4180 parser failed: %s - trying simple split", parse_exc)
                try:
                    first_line = resp.text.strip().split('\n')[0]
                    # Simple split by comma - good enough for basic CSVs
                    headers = [col.strip().strip('"\'') for col in first_line.split(',')]
                    logger.info(
                        "[CSV Headers] ✅ Successfully fetched %d headers via simple parser",
                        len(headers)
                    )
                    return headers
                except Exception as split_exc:
                    logger.error(
                        "[CSV Headers] Both parsers failed - RFC error: %s, Split error: %s",
                        parse_exc, split_exc, exc_info=False
                    )
                    return []
        else:
            logger.warning(
                "[CSV Headers] ❌ SharePoint returned %d %s",
                resp.status_code, resp.reason
            )
            # Log response snippet for debugging
            response_snippet = resp.text[:200] if resp.text else "(no response body)"
            logger.debug("[CSV Headers] Response body: %s", response_snippet)
            
            # Log common error codes
            if resp.status_code == 401:
                logger.error("[CSV Headers] Authentication failed (401) - token may be invalid or expired")
            elif resp.status_code == 403:
                logger.error("[CSV Headers] Access denied (403) - token may lack SharePoint permissions")
            elif resp.status_code == 404:
                logger.error("[CSV Headers] File not found (404) - check CSV path in M expression")
            
            return []
    
    except requests.exceptions.Timeout:
        logger.error("[CSV Headers] ❌ SharePoint request timed out (15s) - network or server latency issue")
    except requests.exceptions.ConnectionError as conn_exc:
        logger.error("[CSV Headers] ❌ Connection error: %s", conn_exc, exc_info=False)
    except requests.exceptions.RequestException as req_exc:
        logger.error("[CSV Headers] ❌ Request exception: %s", req_exc, exc_info=False)
    except Exception as exc:
        logger.error("[CSV Headers] ❌ Unexpected exception: %s", exc, exc_info=True)
    
    return []


def _parse_csv_preview_from_m(expr_str: str) -> List[str]:
    """
    For PromoteHeaders + Csv.Document patterns, attempt to extract column hints
    from downstream M operations that reference the CSV.
    
    Looks for:
    - Table.PromoteHeaders steps that often include metadata
    - AddColumn steps that add explicit columns
    - Explicit column references in TransformColumnTypes
    
    Returns: List of column names (if found)
    """
    columns = []
    
    # Look for explicit column definitions after PromoteHeaders
    # Pattern: PromoteHeaders(...) then operations that name columns
    promote_match = re.search(
        r'(?:Table\.PromoteHeaders|PromotedHeaders)\s*\(\s*([^)]+)\)',
        expr_str
    )
    
    if promote_match:
        # After PromoteHeaders, look for operations that reference columns
        promote_section = expr_str[promote_match.end():]
        
        # Look for Table.RenameColumns - extract all column pairs like {"old", "new"}
        # This handles multi-line and nested structures
        rename_pairs = re.findall(
            r'\{\s*"([^"]+)"\s*,\s*"([^"]+)"\s*\}',
            promote_section
        )
        if rename_pairs:
            # Take the new names (second element in each pair)
            new_columns = [pair[1].strip() for pair in rename_pairs]
            if new_columns:
                columns.extend(new_columns)
                return columns
        
        # Look for Table.SelectColumns that explicitly list columns
        select_matches = re.finditer(
            r'Table\.SelectColumns\s*\(\s*[^,]+\s*,\s*\{([^}]+)\}',
            promote_section,
            re.DOTALL
        )
        ...
        # Look for Table.AddColumn patterns
        add_matches = re.finditer(
            r'Table\.AddColumn\s*\(\s*[^,]+\s*,\s*"([^"]+)"',
            promote_section
        )
    ...
    # Fallback: Look for RenameColumns anywhere in expression if no PromoteHeaders context found
    rename_pairs = re.findall(
    ...
    if rename_pairs:
        for pair_block in rename_pairs:
            for pair in re.finditer(r'\{\s*"[^"]+"\s*,\s*"([^"]+)"\s*\}', pair_block):
                col_name = pair.group(1).strip()
                if col_name and col_name not in columns:
                    columns.append(col_name)
...
def _resolve_schema_universal(
    ...
    3. 🔥 NEW - For SharePoint CSV + PromoteHeaders: fetch actual CSV headers
    4. Infer from source type hints
    5. qlik_fields_map from GetTablesAndKeys (real column names for LOAD * tables)
    6. Last resort: check options metadata
    ...
    # 🔥 Step 2A: DETECT SHAREPOINT CSV WITH PROMOTEHEADERS AND FETCH REAL HEADERS
    # This is the KEY fix for the issue. For SharePoint CSV files with PromoteHeaders,
    # the column names are not available in the M expression — they're resolved at runtime
    # when the CSV is loaded. We must fetch them directly from the CSV file.
    if expr_str and "PromoteHeaders" in expr_str and "Csv.Document" in expr_str:
        logger.info(
            "[_resolve_schema_universal] '%s': 🔥 Detected PromoteHeaders+Csv.Document - "
            "attempting to fetch real CSV headers from SharePoint...",
            table_name,
        )
        
        # Extract SharePoint URL and CSV file path from M expression
        sp_url, csv_path = _extract_sharepoint_csv_path(expr_str)
        
        if sp_url and csv_path:
            logger.info(
                "[_resolve_schema_universal] '%s': Extracted SharePoint components - URL: %s, Path: %s",
                table_name, sp_url[:80], csv_path
            )
            # Get token if available (stored on table or from environment)
            access_token = table.get("access_token", "")
            if not access_token:
                access_token = os.getenv("SHAREPOINT_ACCESS_TOKEN", "")
            
            # If no token yet, try to acquire one with SharePoint-specific scope
            if not access_token:
                logger.info(
                    "[_resolve_schema_universal] '%s': No token provided - "
                    "attempting to acquire SharePoint token with correct scope",
                    table_name
                )
                access_token = _acquire_sharepoint_token(sp_url)
                if not access_token:
                    logger.warning(
                        "[_resolve_schema_universal] '%s': ⚠️  Could not acquire SharePoint token. "
                        "Will attempt to fetch CSV anonymously.",
                        table_name
                    )
            else:
                logger.debug(
                    "[_resolve_schema_universal] '%s': Using provided access token for SharePoint",
                    table_name
                )
            
            # Attempt to fetch headers from SharePoint
            logger.info(
                "[_resolve_schema_universal] '%s': Calling CSV header fetch for URL: %s, path: %s",
                table_name, sp_url, csv_path
            )
            headers = _fetch_csv_headers_from_sharepoint(sp_url, csv_path, access_token)
            
            if headers:
                logger.info(
                    "[_resolve_schema_universal] '%s': ✅ Resolved %d columns from SharePoint CSV headers",
                    table_name, len(headers)
                )
                for col_name in headers:
                    if col_name and col_name.strip():
                        ...
                        columns.append({
                            "name": col_name,
                            "dataType": "string",  # CSV is always text
                ...
                logger.warning(
                    "[_resolve_schema_universal] '%s': ⚠️  CSV header fetch failed. "
                    "Will attempt to extract column hints from M expression patterns...",
                    table_name,
                )
                
                # Try to extract column hints from downstream operations
                preview_cols = _parse_csv_preview_from_m(expr_str)
                if preview_cols:
                    logger.info(
                        "[_resolve_schema_universal] '%s': ✅ Extracted %d columns from M preview hints",
                        table_name, len(preview_cols)
                    )
            ...
            logger.warning(
                "[_resolve_schema_universal] '%s': ⚠️  Could not extract SharePoint URL or CSV path "
                "from M expression. URL: %s, Path: %s. M expression snippet: %s",
                table_name, sp_url, csv_path, expr_str[:150]
            )
    
    # Step 2B: Deep M expression extraction - aggressive regex
    if expr_str:
