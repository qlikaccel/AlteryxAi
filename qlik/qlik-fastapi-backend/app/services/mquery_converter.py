...
class MQueryConverter:
    ...
    #         logger.warning(
    #             "[_detect_and_apply_groupby] Table '%s': is_group_by=True but "
    #             "no group_by_columns and no aggregations found. "
    #             "Skipping Table.Group to prevent empty column output.",
...
def _detect_and_apply_groupby(self, table: Dict[str, Any], prev_step: str, fields: List[Dict]) -> tuple[str, str]:
    ...
    # 🚨 CRITICAL FIX: skip EMPTY group
    if not group_by_cols and not aggregations:
        logger.warning(
            "[_detect_and_apply_groupby] Table '%s': EMPTY Table.Group detected — skipping.",
            table.get("name", "UNKNOWN")
        )
        return "", prev_step
