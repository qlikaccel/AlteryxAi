...
def _llm_settings_for_provider(provider: str) -> dict[str, str]:
    ...
    if provider in {"openai", "gpt"}:
        return {
            ...
            "model": os.getenv("ALTERYX_MQUERY_OPENAI_MODEL") or os.getenv("OPENAI_MODEL") or os.getenv("ALTERYX_MQUERY_LLM_MODEL") or "gpt-4o-mini",
    ...
    if provider in {"anthropic", "claude"}:
        return {
            ...
            "model": os.getenv("ALTERYX_MQUERY_ANTHROPIC_MODEL") or os.getenv("ANTHROPIC_MODEL") or "claude-3-5-sonnet-latest",
            "url": os.getenv("ALTERYX_MQUERY_ANTHROPIC_URL") or os.getenv("ANTHROPIC_BASE_URL") or "https://api.anthropic.com/v1/messages",
        }
    return {
        "provider": "huggingface",
        "token": os.getenv("ALTERYX_MQUERY_LLM_API_KEY") or os.getenv("HF_TOKEN") or "",
        "model": os.getenv("ALTERYX_MQUERY_LLM_MODEL") or "meta-llama/Llama-3.1-8B-Instruct",
...
def _llm_settings() -> dict[str, str]:
    provider = (os.getenv("ALTERYX_MQUERY_LLM_PROVIDER") or "").strip().lower()
    if not provider:
...
def _parse_macro_config(node: dict[str, Any]) -> dict[str, str]:
    """Extract macro configuration from ToolContainer node."""
    config = node.get("config") if isinstance(node.get("config"), dict) else {}
    
    # Try structured config first
    if config.get("macroType") or config.get("macroPath"):
        return {
            "macroType": str(config.get("macroType") or ""),
            "macroPath": str(config.get("macroPath") or ""),
            "controlParameter": str(config.get("controlParameter") or ""),
            "iterationLimit": str(config.get("iterationLimit") or ""),
            "stopCondition": str(config.get("stopCondition") or ""),
        }
    
    # Fall back to parsing configurationText
    # Format: [0] ID [1] plugin [2] X [3] Y [4] MacroType [5]+ depends on type
    lines = _config_lines(node)
    start_idx = 4 if len(lines) > 4 and lines[0].isdigit() else 0
    
    macro_type = lines[start_idx] if start_idx < len(lines) else ""
    control_param = ""
    iteration_limit = ""
    stop_condition = ""
    macro_path = ""
    
    if macro_type.lower() == "standard":
        # Standard: [4] Standard [5] macroPath [6] label ...
        macro_path = lines[start_idx + 1] if start_idx + 1 < len(lines) else ""
    elif macro_type.lower() == "batch":
        # Batch: [4] Batch [5] controlParam [6] macroPath [7] label ...
        control_param = lines[start_idx + 1] if start_idx + 1 < len(lines) else ""
        macro_path = lines[start_idx + 2] if start_idx + 2 < len(lines) else ""
    elif macro_type.lower() == "iterative":
        # Iterative: [4] Iterative [5] iterationLimit [6] stopCondition [7] macroPath [8] label ...
        iteration_limit = lines[start_idx + 1] if start_idx + 1 < len(lines) else ""
        stop_condition = lines[start_idx + 2] if start_idx + 2 < len(lines) else ""
        macro_path = lines[start_idx + 3] if start_idx + 3 < len(lines) else ""
...
def _inline_macro_from_path(macro_path: str, macro_type: str, control_param: str, iteration_limit: str, current_stream: str) -> str:
    """
    Attempt to read and inline a macro file. 
    Returns M expression or empty string if macro file cannot be read.
    """
    # Try to find and read the macro file
    try:
        # Construct possible paths
        possible_paths = [
            macro_path,
            os.path.join("macros", os.path.basename(macro_path)) if not macro_path.startswith("macros") else macro_path,
        ]
        ...
        if not macro_content:
            # Cannot read macro file - return empty to trigger fallback
            return ""
        
        # For Standard macros: pass through the current stream
        if macro_type.lower() == "standard":
            # Standard macros are typically pass-through transformations
            # Without parsing the .yxmc file, we pass the current stream and add a comment
            return current_stream
        
        # For Batch macros: iterate over control parameter values
        elif macro_type.lower() == "batch" and control_param:
            # Group by control parameter and process each batch
            grouped = f'Table.Group({current_stream}, {{"{_quoted(control_param)}"}}' + ', {{"Batch", each _, type table}})'
            return f'Table.Combine(Table.TransformColumns({grouped}, {{"Batch", each _, type table}}))'
        
        # For Iterative macros: apply recursively until condition is met
        elif macro_type.lower() == "iterative":
            max_iterations = int(iteration_limit) if iteration_limit.isdigit() else 100
            # Simple recursive expansion - apply the macro step repeatedly
            return f"// Iterative macro: {macro_path} (max {max_iterations} iterations) - manual implementation required"
        
        return ""
    
    except Exception as e:
        logger.warning(f"Failed to inline macro from {macro_path}: {str(e)}")
        return ""
...
def _summarize_config(node: dict[str, Any]) -> tuple[list[str], list[dict[str, str]]]:
    ...
    # Skip header lines (ID, plugin class, X, Y coordinates)
    # Configuration data starts at index 4 or later
    start_index = 4 if len(lines) > 4 and lines[0].isdigit() and "plugin" in lines[1].lower() else 0
    ...
    while index + 1 < len(lines):
        ...
        if action_lower == "groupby":
            ...
            index += 2
        elif action_lower in {"sum", "count", "average", "avg", "min", "max", "countdistinct"}:
...
def _step_for_tool(
    ...
    if tool_key == "summarize":
        ...
        for agg in aggregations:
            ...
            if action == "sum":
                agg_parts.append(f'{{"{rename}", each List.Sum(List.Transform([{field}], Number.From)), type number}}')
                ...
                agg_parts.append(f'{{"{rename}", each (List.Distinct([{field}]) as L => List.Count(L)), Int64.Type}}')
            elif action == "min":
    ...
    if tool_key == "toolcontainer":
        # Handle Alteryx Macro tools (Standard, Batch, Iterative)
        macro_info = _parse_macro_config(node)
        macro_type = macro_info.get("macroType", "")
        macro_path = macro_info.get("macroPath", "")
        control_param = macro_info.get("controlParameter", "")
        iteration_limit = macro_info.get("iterationLimit", "")
        
        # Try to read and inline the macro file
        if macro_path:
            inline_expr = _inline_macro_from_path(macro_path, macro_type, control_param, iteration_limit, current)
            if inline_expr:
                return _next_name(f"Macro_{macro_type or 'Invoked'}", index), inline_expr, comment + f" - inlined {macro_type} macro from {macro_path}"
        
        # Fallback: passthrough with warning for macro
        warning_msg = f"// AlteryxGuiToolkit.ToolContainer.ToolContainer ({macro_type} Macro) requires manual mapping."
        return _next_name("MacroPassthrough", index), current, warning_msg + f" Macro: {macro_path} | Type: {macro_type}"
