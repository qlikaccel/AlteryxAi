...
# ── CSV column definitions for Alteryx SharePoint tables ──
_CSV_COLUMNS = {
    "orders_AI.csv":         ["OrderID", "CustomerID", "ProductID", "OrderAmount", "OrderDate", "Channel"],
    "region_parameters.csv": ["Region", "Manager", "TaxRate"],
    "hierarchy.csv":         ["NodeID", "ParentID", "NodeName", "Level"],
}
...
async def publish_mquery_endpoint(request: PublishMQueryRequest):
    ...
    try:
        ...
        if raw_script.strip():
            ...
            # ✅ Auto-build qlik_fields_map from known CSV column definitions
            if not qlik_fields_map:
                ...
                from app.services.powerbi_publisher import _extract_typedarticle_columns
                for tbl in tables_m:
                    tbl_name = tbl.get("name", "")
                    expr = tbl.get("m_expression", "")
                    cols = _extract_typedarticle_columns(expr)
                    if not cols:
                        m = re.search(r'\[Name\]\s*=\s*"([^"]+\.csv)"', expr)
                        if m:
                            cols = _CSV_COLUMNS.get(m.group(1).strip(), [])
                    if cols:
                        qlik_fields_map[tbl_name] = cols
                        logger.info("[publish_mquery] CSV columns mapped: '%s' -> %d cols", tbl_name, len(cols))

            # Fix broken Table.Group(Input, {}, {}) — destroys all columns
            for tbl in tables_m:
                expr = tbl.get("m_expression", "")
                if "Table.Group(Input, {}, {})" in expr:
                    tbl["m_expression"] = re.sub(
                        r'GroupedData_5\s*=.*?Table\.Group\(Input,\s*\{\},\s*\{\}\).*?Checkpoint_6\s*=\s*GroupedData_5',
                        'Checkpoint_6 = MacroPassthrough_4',
                        expr,
                        flags=re.DOTALL
                    )
                    logger.info("[publish_mquery] Fixed broken Table.Group in '%s'", tbl.get("name"))
    ...
    # ── Fix: inject CSV columns and repair Table.Group ──
        for tbl in tables_m:
            tbl_name = tbl.get("name", "")
            expr = tbl.get("m_expression", "")
            if not qlik_fields_map.get(tbl_name):
                m = re.search(r'\[Name\]\s*=\s*"([^"]+\.csv)"', expr)
                if m:
                    cols = _CSV_COLUMNS.get(m.group(1).strip(), [])
                    if cols:
            ...
            if "Table.Group(Input, {}, {})" in expr:
                ...
                logger.info("[publish_mquery] Fixed Table.Group in '%s'", tbl_name)
